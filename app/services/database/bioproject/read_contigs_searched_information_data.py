...
def read_contigs_searched_information_data(search_data):
    ...
    region_ids = get_region_all_ids(search_data['srp'])
    resfinder_ids = get_resfinder_all_ids(search_data['srp'])
    vfdb_ids = get_vfdb_all_ids(search_data['srp'])
    combined_list = region_ids + resfinder_ids + vfdb_ids
    list_set = list(set(combined_list))
        
    contigs = []
    for seq_record in sequences:
        if seq_record.id in list_set:
            contigs.append({
                'ID': seq_record.id,
